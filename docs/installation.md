...
The main branch ships 0.1.2 under LGPL-3.0-only WITH LGPL-3.0-linking-exception, published to registries.

This guide describes version 0.1.2. Check the linked registry or release
...
Deno can use `npm:@corbet-labs/cgrade`. The browser export bundles runtime
...
[PyPI](https://pypi.org/project/cgrade/0.1.2/). Install this release with pip
or uv in your Python environment:

```sh
python -m pip install cgrade==0.1.2
```

```sh
uv pip install cgrade==0.1.2
```

For a uv project, `uv add cgrade==0.1.2` adds the package to your dependencies.
...
For an isolated CLI environment, use `pipx install cgrade==0.1.2` or
`uv tool install cgrade==0.1.2`.
...
[GitHub release](https://github.com/corbet-foss/cgrade/releases/tag/v0.1.2).

## JSR

The JSR package is
[`@corbet-labs/cgrade`](https://jsr.io/@corbet-labs/cgrade@0.1.2):

```sh
deno add jsr:@corbet-labs/cgrade@0.1.2
```

## Typst

Download `cgrade-0.1.2-typst.tar.gz` from the matching GitHub release and
extract its contents into `typst/packages/local/cgrade/0.1.2` under your

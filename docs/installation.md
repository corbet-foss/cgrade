...
[PyPI](https://pypi.org/project/cgrade/0.1.1/). Install this release with pip
or uv in your Python environment:

```sh
python -m pip install cgrade==0.1.1
```

```sh
uv pip install cgrade==0.1.1
```

For a uv project, `uv add cgrade==0.1.1` adds the package to your dependencies.
...
For an isolated CLI environment, use `pipx install cgrade==0.1.1` or
`uv tool install cgrade==0.1.1`.
...
[GitHub release](https://github.com/corbet-foss/cgrade/releases/tag/v0.1.1).

## JSR

The JSR package is
[`@corbet-labs/cgrade`](https://jsr.io/@corbet-labs/cgrade@0.1.1):

```sh
deno add jsr:@corbet-labs/cgrade@0.1.1
```

## Typst

Download `cgrade-0.1.1-typst.tar.gz` from the matching GitHub release and
extract its contents into `typst/packages/local/cgrade/0.1.1` under your

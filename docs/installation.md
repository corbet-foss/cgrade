...
Deno can use `npm:@corbet-foss/cgrade`. The browser export bundles runtime
...
[PyPI](https://pypi.org/project/cgrade/0.1.0/). Install this release with pip
or uv in your Python environment:

```sh
python -m pip install cgrade==0.1.0
```

```sh
uv pip install cgrade==0.1.0
```

For a uv project, `uv add cgrade==0.1.0` adds the package to your dependencies.
...
For an isolated CLI environment, use `pipx install cgrade==0.1.0` or
`uv tool install cgrade==0.1.0`.
...
[GitHub release](https://github.com/corbet-labs/cgrade/releases/tag/v0.1.0).

## JSR

The JSR package is
[`@corbet-foss/cgrade`](https://jsr.io/@corbet-foss/cgrade@0.1.0):

```sh
deno add jsr:@corbet-foss/cgrade@0.1.0
```

## Typst

Download `cgrade-0.1.0-typst.tar.gz` from the matching GitHub release and
extract its contents into `typst/packages/local/cgrade/0.1.0` under your
